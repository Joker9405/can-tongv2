...
export default function App() {
  ...
  const handleSearch = async (term: string) => {
    ...
    if (!query) {
      setMatchedEntries([]); setSelectedEntry(null); setRelatedWords([]); setNotFound(false); setSwearingToggle(false);
      return;
    }
    ...
    // 1. 自动判断是否命中词典
    const isHit = searchResults.length > 0;

    // 2. 将搜索词和命中结果发送给统一的后端接口
    // 这样后端就能根据 isHit 决定存入哪张表
    fetch('/api/search', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ q: query, isHit: isHit })
    ...
    if (isHit) {
      ...
      } else { setRelatedWords([]); }
    } else {
      setMatchedEntries([]); setSelectedEntry(null); setRelatedWords([]); setNotFound(true); setSwearingToggle(false);
    }
  };
  ...
  const toggleLanguage = () => setLanguage(prev => prev === 'chs' ? 'en' : 'chs');
          ...
          <div className="mt-2 space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {colloquialEntries.filter(e => e !== selectedEntry).map((entry, index) => (
                <button key={`green-${index}`} onClick={() => handleEntryClick(entry)} className="px-5 py-3 bg-[#c8ff00] text-black rounded-[28px] p-8 relative text-lg hover:scale-105 transition-transform font-medium">{entry.zhh}</button>
              ))}
            </div>

            {vulgarEntries.length > 0 && !swearingToggle && (
              <div className="flex items-center gap-2">
                <button onClick={() => setSwearingToggle(true)} className="px-5 py-3 bg-[#ff0090] text-white rounded-[28px] p-8 relative text-lg hover:bg-[#ff1a9f] transition-colors font-medium font-bold font-[Anton]">Swearing</button>
              </div>
            )}
            ...
            {swearingToggle && vulgarEntries.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {vulgarEntries.filter(e => e !== selectedEntry).map((entry, index) => (
                  <button key={`magenta-${index}`} onClick={() => handleEntryClick(entry)} className="px-5 py-3 bg-[#ff0090] text-white rounded-[28px] p-8 relative text-lg hover:scale-105 transition-transform font-medium animate-slide-in" style={{ animationDelay: `${index * 0.05}s` }}>{entry.zhh}</button>
                ))}
              </div>
            )}

            <div className="relative">
              <button onClick={() => setShowAddDrawer(true)} className="px-5 py-3 bg-gray-700 text-[#c8ff00] rounded-[28px] p-8 relative text-lg hover:bg-gray-600 transition-colors font-medium font-[Anton] font-bold">add</button>
